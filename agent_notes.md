...
## P12 — output/identity refinements (Wes's field test on pipecat)

- Every command now takes `--json` (`audit` and `identities` added); text views drop the opaque `mass` number (ordering unchanged; JSON keeps it as the sortable) and print "top-3 on N files (share) e.g. best-file".
